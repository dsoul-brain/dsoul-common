...
import com.easwift.basic.dataaccess.api.IdGenerator;
import com.easwift.basic.dataaccess.api.SqlDescription;
import com.easwift.basic.dataaccess.exception.DaoException;
import com.easwift.basic.util.BasicMapUtils;

/**
 * @descrpiton [数据访问对象基类] <br/>
 *             提供了数据库访问的基础方法 <br/>
 *             如果这些方法都无法满足业务需求,可以考虑使用该类提供的 sqlSessionTemplate, jdbcTemplate.
 * @author [dingwei.tang]
 * @date [2017年2月9日 下午1:44:28]
 */
@Transactional
public class BasicDaoSupport extends BasicAbstractDao {

	private static final Logger logger = Logger.getLogger(BasicDaoSupport.class);
	...
	public List<Map<String, Object>> selectForCamelCaseMap(String statementName) throws DaoException {
		...
		if (ClassUtils.isAssignable(element.getClass(), Map.class)) {
			BasicMapUtils.keyToCamelCase((List<Map<String, Object>>) resultList);
