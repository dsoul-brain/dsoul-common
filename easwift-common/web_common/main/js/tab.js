	...
	function e(n) {
		...
		if (a.current == null || a.current != this) {
			$(p).mouseover(function() {
				...
				// 主页点击右侧页签，左侧菜单与之对应
				var id = p.attr("id");
				$(".nav-list ul", parent.document).each(function(i) {
					$(this).find("[class='active']").removeClass();
				});
				$("#sidebar",parent.document).find(".submenu").hide();
				$("#"+id,parent.document).parent("ul").show();
				$("#"+id, parent.document).attr("class","active");
			});
