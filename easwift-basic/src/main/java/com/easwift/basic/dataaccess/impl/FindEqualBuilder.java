...
/**
 * 
 * @descrpiton [自定义约束构建] 根据值对象中的非空属性作为条件进行查询<br/>
 *             ignores 为忽略的查询属性<br/>
 *             mapping 为实体类属性(key)和值对象属性(value)映射
 * @author [dingwei.tang]
 * @date [2017年2月9日 下午1:37:15]
 */
public class FindEqualBuilder {
	...
	private static final Logger logger = Logger.getLogger(FindEqualBuilder.class);
	...
	public void init() {
		...
		try {
			PropertyDescriptor[] propertyDescriptors = PropertyUtils.getPropertyDescriptors(paramObj.getClass());
			for (PropertyDescriptor pd : propertyDescriptors) {
				...
				if (!isInIgnore(propName)) {
					Object value = pd.getReadMethod().invoke(paramObj, ArrayUtils.EMPTY_OBJECT_ARRAY);
					if (value != null) {
	...
	public FindEqualBuilder addMapping(String propertyName, String paramPropertyName) {
		if (mapping == null) {
